...
Two sets of observations are *paired*\index{paired data} if each observation in one set has a special correspondence or connection with exactly one observation in the other dataset.
:::
...
The key to working with paired data (for bootstrapping and mathematical approaches) is to consider the measurement of interest to be the difference in measured values across the pair of observations.\index{bootstrap!CI paired difference}\index{bootstrap CI paired difference}
...
When two sets of observations have this special correspondence, they are said to be **paired**\index{paired data}.
...
Using the histogram of bootstrapped difference in means, estimate the standard error of the mean of the sample differences, $\bar{x}_{diff}.$

::: {.callout-note collapse="true"}
## Solution

The bootstrapped differences in sample means vary roughly from 0.7 to 7.5, a range of \$6.80.
...
The T score\index{T score!paired difference}\index{paired difference!T score} is a ratio of how the sample mean difference varies from zero as compared to how the observations vary.
...
::: {.workedexample data-latex=""}
Create a 95% confidence interval\index{t-CI!paired difference} for the average price difference between books at the UCLA bookstore and books on Amazon.
...
Should UCLA students always buy their books on Amazon?

::: {.callout-note collapse="true"}
## Solution

The average price difference is only mildly useful for this question.
