...
Although the ideas in this chapter are quite similar to the t-test, they have earned themselves their own name: **AN**alysis **O**f **VA**riance\index{Analysis of Variance}, or ANOVA\index{ANOVA}.
:::
...
Six of the 429 cases represented in `mlb_players_18` are shown in @tbl-mlbBat18DataFrame, and descriptions for each variable are provided in @tbl-mlbBat18Variables.
...
The null hypothesis under consideration is the following: $\mu_{OF} = \mu_{IF} = \mu_{C} % = \mu_{DH}.$ Write the null and corresponding alternative hypotheses in plain language.
...
@tbl-mlbHRPerABSummaryTable provides summary statistics for each group.
A side-by-side box plots for the on-base percentage is shown in @fig-mlbANOVABoxPlot.
...
This is called **data snooping**\index{data snooping} or **data fishing**\index{data fishing}.
Naturally, we would pick the groups with the large differences for the formal test, and this would lead to an inflation in the Type I error rate.
To understand inflated Type I error rates better, let's consider a slightly different problem.
...
If we select only the classes that look different and then perform a formal test, we will probably make the wrong conclusion that the assignment wasn't random.
...
We call this variability the **mean square between groups (MSG)**\index{mean square between groups} $(MSG)$, and it has an associated degrees of freedom, $df_{G} = k - 1$ when there are $k$ groups.
...
To this end, we compute a pooled variance estimate, often abbreviated as the **mean square error** $(MSE)$\index{mean square error}, which has an associated degrees of freedom value $df_E = n - k.$ It is helpful to think of $MSE$ as a measure of the variability within the groups.
...
We recall the exams from @sec-rand2mean which demonstrated a two-sample randomization test for a comparison of means.
Suppose now that the teacher had had such an extremely large class that three different exams were given: A, B, and C.
@tbl-summaryStatsForThreeVersionsOfExams and @fig-boxplotThreeVersionsOfExams provide a summary of the data including exam C.
...
@fig-randANOVA shows the process of randomizing the three different exams to the observed exam scores.
...
There is only one iteration of the randomization process in @fig-randANOVA, leading to three different randomized sample means (computed assuming the null hypothesis is true).
...
Building on @fig-randANOVA, @fig-rand3exams shows the values of the simulated $F$ statistics over 1,000 random simulations.
...
While it is temping to say that exam C is harder than the other two (given the inability to differentiate between exam A and exam B in @sec-rand2mean), we must be very careful about conclusions made using different techniques on the same data.
...
We return to the baseball example from @tbl-mlbHRPerABSummaryTable to demonstrate the mathematical model applied to the ANOVA setting.
...
There are $k = 3$ groups, so $df_{G} = k - 1 = 2.$ There are $n = n_1 + n_2 + n_3 = 429$ total observations, so $df_{E} = n - k = 426.$ Then the $F$-statistic is computed as the ratio of $MSG$ and $MSE:$ $F = \frac{MSG}{MSE} = \frac{0.00803}{0.00158} = 5.082 \approx 5.077.$ $(F = 5.077$ was computed by using values for $MSG$ and $MSE$ that were not rounded.)
:::
:::
...
::: {.workedexample data-latex=""}
The p-value corresponding to the shaded area in @fig-fDist2And423Shaded is equal to about 0.0066.
...
As we saw at the end of @sec-randANOVA, the follow-up questions surrounding individual group comparisons is called a problem of **multiple comparisons**\index{multiple comparisons} and is outside the scope of this text.
...
@tbl-anovaSummaryTableForOBPAgainstPosition shows an ANOVA summary to test whether the mean of on-base percentage varies by player positions in the MLB.
...
-   **Approximately normal.** As with one- and two-sample testing for means, the normality assumption is especially important when the sample size is quite small when it is ironically difficult to check for non-normality.
    A histogram of the observations from each group is shown in @fig-mlbANOVADiagNormalityGroups.
...
-   **Constant variance.** The last assumption is that the variance in the groups is about equal from one group to the next. This assumption can be checked by examining side-by-side box plots of the outcomes across the groups, as in @fig-mlbANOVABoxPlot. In this case, the variability is similar in the four groups but not identical. We see in @tbl-mlbHRPerABSummaryTable that the standard deviation does not vary much from one group to the next.
