...
2.  **The sample is large enough.** The sample size cannot be too small. What qualifies as "small" differs from one context to the next, and we'll provide suitable guidelines for proportions in [Chapter -@sec-inference-one-prop].
...
::: {.guidedpractice data-latex=""}
Use Sian's ACT score, 24, along with the ACT mean and standard deviation to compute their Z score.

::: {.callout-note collapse="true"}
## Solution

$Z_{Sian} = \frac{x_{Sian} - \mu_{ACT}}{\sigma_{ACT}} = \frac{24 - 21}{5} = 0.6$
:::
:::
...
If 84% had lower scores than Nel, the number of people who had better scores must be 16%.
...
We found the probability to be 0.6664.
...
(b) What is the $97.5^{th}$ percentile of the male heights? As always with normal probability problems, first draw a picture.
...
::: {.guidedpractice data-latex=""}
Find the percent of SAT takers who earn between 1500 and 2000.
...
The distance given by $z^\star \times SE$ is called the **margin of error**\index{margin of error}.
...
### Variability of the statistic {#sec-variability-opportunity-cost}
...
### Observed statistic vs. null statistics {#sec-observed-vs-null-opportunity-cost}
...
### Variability of the statistic {#sec-variability-medical-consultant}
...
### Observed statistic vs. null statistics {#sec-observed-vs-null-medical-consultant}
...
Thus, a **95% confidence interval**\index{95\% confidence interval}\index{confidence interval} for such a point estimate can be constructed:

\vspace{-5mm}

$$\text{point estimate} \pm 1.96 \times SE$$

We can be **95% confident**\index{95\% confident} this interval captures the true value.
:::
...
::: {.workedexample data-latex=""}
The point estimate in the opportunity cost study was that 20% fewer students would buy a video if they were reminded that money not spent now could be spent later on something else.
...
### Variability of the statistic {#sec-variability-stent}
...
::: {.guidedpractice data-latex=""}
In @fig-95PercentConfidenceInterval, one interval does not contain the true proportion, $p = 0.3.$ Does this imply that there was a problem with the datasets that were selected?
...
```{r}
#| include: false
# Reorganize terms to display in specific 3-column format
# Column 1: 95% confidence interval, 95% confident, Central Limit Theorem, margin of error, normal curve
# Column 2: normal distribution, normal model, normal probability table, null distribution, parameter  
# Column 3: percentile, sampling distribution, standard error, standard normal distribution, Z score
terms_chp_13 <- c(
  "95% confidence interval", "95% confident", "Central Limit Theorem", "margin of error", "normal curve",
  "normal distribution", "normal model", "normal probability table", "null distribution", "parameter",
  "percentile", "sampling distribution", "standard error", "standard normal distribution", "Z score"
)
```

The terms introduced in this chapter are presented in @tbl-terms-chp-13.
...
[^13-foundations-mathematical-2]: Sometimes also called the Gaussian distribution after mathematician Carl Friedrich Gauss.

[^13-foundations-mathematical-10]: Source: CDC data.
